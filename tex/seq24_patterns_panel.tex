...
\label{sec:seq24_patterns_panel}
   ...
   \textsl{Seq24} in "live mode".  The musician can
   ...
   each pattern, and \textsl{Seq24} runs in "song mode".

   However, if \textsl{Seq24} is using JACK transport, then, instead of
   the behavior described above, live versus song mode is controlled by the
   JACK start mode option (see the \textbf{JACK Start mode} item.
...
\end{figure}
   ...
   These characters can be shown in each pattern.
...
\end{figure}
   ...
   We have found that one can also use normal keys to enable queueing.
