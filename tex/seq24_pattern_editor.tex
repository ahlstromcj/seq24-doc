...
\end{figure}
   ...
   Also, a separate section for editor-wide options is provided.
...
\end{figure}
   ...
   You may want to check out the \textsl{Sequencer64} document, which includes
   many things learned while "rebooting" the \textsl{Seq24} project.

\subsection{Pattern Editor / Common Actions}
\label{subsec:seq24_pattern_editor_common}

   This section is a catch-all for actions not described above.

\subsubsection{Pattern Editor / Common Actions / Scrolling}
\label{subsec:seq24_pattern_editor_scrolling}

   Let's describe the actions that can be performed with a
...
\label{subsec:seq24_pattern_editor_close}
   ...
   \textsl{Seq24} also provides the Ctrl-W key to close the pattern
   editor window.
   \index{window!close}

   However, be aware that this convention does not apply to the other
   application windows of \textsl{Seq24}.
